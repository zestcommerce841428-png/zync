...
export async function createContactGroup(
  ...
  const group: ContactGroup = { id, ownerId, name, emails: [...new Set(emails)], createdAt: new Date().toISOString() }
...
export async function getContactGroup(id: string): Promise<ContactGroup | null> {
  const redis = getRedisClient()
  const raw = await redis.get(KEY(id))
  if (!raw) return null
  try { return JSON.parse(raw) as ContactGroup } catch { return null }
}

export async function updateContactGroup(id: string, patch: Partial<ContactGroup>): Promise<void> {
  const redis = getRedisClient()
...
export async function deleteContactGroup(id: string, ownerId: string): Promise<void> {
  const redis = getRedisClient()
...
export async function listContactGroups(ownerId: string): Promise<ContactGroup[]> {
  const redis = getRedisClient()
