...
export default function TransferHistoryPage(): React.ReactElement {
  ...
  React.useEffect(() => {
    fetch('/api/transfer/history')
      .then((r) => {
        if (r.status === 401) throw new Error('Sign in to view your transfer history.')
        return r.json()
      })
      .then(setTransfers)
  ...
  const deleteTransfer = async (slug: string) => {
    ...
    <Container maxWidth="md" sx={{ py: { xs: 6, md: 10 } }}>
      <Stack spacing={1} sx={{ mb: 4 }}>
        ...
        <Typography color="text.secondary">
          Active transfers you've sent. Links expire automatically and files are deleted.
        </Typography>
      </Stack>
                ...
                <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                  <Stack spacing={1.5}>
                    <Stack direction="row" sx={{ alignItems: 'flex-start', justifyContent: 'space-between', gap: 1 }}>
                      ...
                      <Stack direction="row" spacing={0.5} sx={{ flexShrink: 0 }}>
                        <Tooltip title="Open download page">
                          <IconButton size="small" href={url} target="_blank" rel="noopener">
                            <OpenInNewIcon fontSize="small" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={copied === t.slug ? 'Copied!' : 'Copy link'}>
                          <IconButton size="small" onClick={() => copyLink(t.slug)} color={copied === t.slug ? 'success' : 'default'}>
                    ...
                    <Stack direction="row" spacing={0.75} sx={{ flexWrap: 'wrap' }}>
                      <Chip
                        icon={<AccessTimeIcon />}
                        label={days > 0 ? `${days}d left` : 'Expiring'}
                        size="small"
                        color={days <= 2 ? 'warning' : 'default'}
                      />
                      <Chip
                      ...
                      <Chip
                        ...
                        size="small"
                        color={t.maxDownloads && t.downloadCount >= t.maxDownloads ? 'error' : 'default'}
                      />
                      {t.passwordProtected && (
